...
const STORAGE_KEY = 'lernfeld:persistent-drawer';
const ACCORDION_KEY = 'lernfeld:persistent-accordion';
    ...
    <Box sx={{ display: 'flex', width: '100%' }}>
      ...
      <Main open={open}>
        ...
        <Box sx={{ maxWidth: '100%', mx: 'auto', width: '100%' }}>
          {pathname && pathname.startsWith('/lernfeld/') && (
            <Box sx={{ mb: 3, mt: 2, px: { xs: 0.5, md: 1 }, display: 'flex', justifyContent: 'center' }}>
              <Breadcrumbs aria-label="breadcrumb">
                ...
                {pathname.split('/').filter(Boolean).slice(1).map((segment, index, arr) => {
                  const href = '/lernfeld/' + arr.slice(0, index + 1).join('/');
