...
function renderStructuredText(
  ...
  return lines.map((line, lineIdx) => {
    const segments = splitLeadingLabel ? splitByLeadingLabelColon(line) : [{ text: line, bold: false }];
...
function questionSummaryText(question: Karte, defaultIndex: number) {
  const lines = linesAfterInsertGorunce(question.frageDE ?? question.frageTR ?? '');
...
export default function KarteQuestionsAccordion({
  ...
  React.useEffect(() => {
    ...
    setExpanded(prev => {
      ...
      const exists = questions.some((_, idx) => `panel-${startIndex + idx}` === prev);
        ...
        return (
          <Accordion key={panelId} expanded={expanded === panelId} onChange={handleChange(panelId)}>
            <AccordionSummary aria-controls={`${panelId}-content`} id={`${panelId}-header`}>
              <Typography component="span" fontWeight={600} sx={{ minWidth: 48 }}>
                {position}.
              </Typography>
