...
export type NavNode =
  ...
  | { id: string; type: 'dir'; name: string; href: string; children: NavNode[] };

export type PersistentDrawerConfig = {
  storageKey: string;
  accordionKey: string;
  pathPrefix: string;
...
export default function PersistentDrawer({ nodes, children, config }: Props) {
  ...
  React.useEffect(() => {
    const stored = typeof window === 'undefined' ? null : window.localStorage.getItem(config.storageKey);
    if (stored === 'open') {
      setOpen(true);
    }
  }, [config.storageKey]);
  ...
  React.useEffect(() => {
    ...
    window.localStorage.setItem(config.storageKey, open ? 'open' : 'closed');
  ...
  React.useEffect(() => {
    ...
    setExpanded(prev => {
      ...
      const stored = window.localStorage.getItem(config.accordionKey);
  ...
  }, [nodes, config.accordionKey]);

  React.useEffect(() => {
    if (typeof window === 'undefined') return;
    if (expanded) {
      window.localStorage.setItem(config.accordionKey, expanded);
    } else {
      window.localStorage.removeItem(config.accordionKey);
    }
  }, [expanded, config.accordionKey]);

  const normalizedPath = normalizePath(pathname ?? '');

  const renderListItems = React.useCallback(
    (items: NavNode[], depth = 0): React.ReactNode[] =>
      items.map(node => {
        if (node.type === 'dir') {
            ...
            <React.Fragment key={node.id}>
              <ListItem disablePadding>
                ...
                >
                  ...
                  <ListItemText
                    primary={config.formatLabel(node.name)}
        ...
        return (
          <ListItem disablePadding key={node.id}>
            ...
            >
              ...
              <ListItemText primary={config.formatLabel(node.name)} />
            </ListItemButton>
          </ListItem>
        );
      }),
    ...
    <Box sx={{ display: 'flex', width: '100%' }}>
      ...
      <Main open={open}>
        ...
        <Box sx={{ maxWidth: '100%', mx: 'auto', width: '100%' }}>
          {pathname && pathname.startsWith(config.pathPrefix) && (
            <Box sx={{ mb: 3, mt: 2, px: { xs: 0.5, md: 1 }, display: 'flex', justifyContent: 'center' }}>
              <Breadcrumbs aria-label="breadcrumb">
                ...
                {pathname.split('/').filter(Boolean).slice(1).map((segment, index, arr) => {
                  const href = config.pathPrefix + arr.slice(0, index + 1).join('/');
                  const isLast = index === arr.length - 1;
                  
                  return isLast ? (
                    <Typography key={href} color="text.primary">
                      {config.formatLabel(segment)}
                    </Typography>
                  ) : (
                    <MuiLink
                      key={href}
