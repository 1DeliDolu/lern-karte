...
export function findDocNodeBySegments(
  nodes: DocNode[],
  segments: string[]
): DocNode | null {
...
export function listDocChildrenBySegments(
  nodes: DocNode[],
  segments: string[]
): DocNode[] {
...
export function readDoc(
  category: string,
  slugOrSegments: string | string[]
): DocFile {
  const rawParts = Array.isArray(slugOrSegments)
    ? slugOrSegments
    : [slugOrSegments];
  ...
  for (let len = parts.length - 1; len >= 1; len--) {
    ...
    try {
      ...
      if (fs.existsSync(dir)) {
        ...
        const match = files.find((f) => f.toLowerCase() === wanted);
        if (match)
          return tryRead(path.join(dir, match), "stripped-case-insensitive");
      }
    } catch {
      // ignore
    }
  ...
  try {
    ...
    if (candidates.length) {
      ...
      const threshold = Math.min(
        3,
        Math.floor(Math.max(best.name.length, last.length) * 0.35)
      );
